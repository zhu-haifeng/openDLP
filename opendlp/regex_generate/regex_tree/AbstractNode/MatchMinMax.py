...
class MatchMinMax (TernaryOperator):
    ...
    def is_valid(self):
        ...
        validFirst = first.is_valid() and ~(isinstance(first, Concatenator) or isinstance(first,Quantifiers) or isinstance(first,MatchMinMax) or isinstance(first,MatchMinMaxGreedy) or isinstance(first, Anchor) or isinstance(first, Lookaround))
